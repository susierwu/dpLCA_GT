...
def parse_scenario_year_from_elec(path: str) -> Tuple[str, int]:
    ...
    Expects names like: elec_ssp119-2040.xlsx or elec_SSP5-H_2050.xlsx.
    Returns
    -------
    scenario : str   (e.g. "ssp119", "ssp2-m")
    year     : int   (e.g. 2040)
    """
    fname = os.path.basename(path)
    # Grab bit between 'elec_' and last '-' as scenario, then year at end
    m = re.match(r"elec_(.+)-(\d{4})\.xlsx", fname, flags=re.IGNORECASE)
    if not m:
        raise ValueError(f"Cannot parse scenario/year from '{fname}'")
    scenario_raw, year_str = m.groups()
    return scenario_raw.lower(), int(year_str)
...
# -----------------------------------------------------------------------------
# 4. Dynamic LCIA computation (central + uncertainty) by category
# -----------------------------------------------------------------------------
...
def plot_multi_scenario_split_each_ghg_totalscore(
    results: dict,
    metric: str = "rf",
    gases_to_plot: list = None,     # default → only totals
