...
class majorghg_get_f:
    ...
    def get_gas_deltaC_t(self, specie: str, pulse_size: float = 1.0, store: bool = True):
        ...
        # --- baseline concentration (timebounds) ---
        conc_base = np.asarray(
            self.f.concentration.loc[dict(scenario=self.scn, specie=specie)].values
        )
        ...
        if getattr(self, "debug", True):
            ...
            print(
                f"[pIRF-debug] scn={self.scn} MY={pulse_year} gas={specie} "
                f"tb_pulse={tb_pulse} tp_pulse={tp_pulse} tb_resp0={tb_resp0} "
