...
class majorghg_get_f:
    ...
    def __init__(
        ...
        # pIRF diagnostic printing (set to False to silence)
        self.debug = True
    ...
    def _get_pulse_year(self) -> int:
        """
        Pulse year as a *calendar year* corresponding to ``year_index``.

        Robust mapping:
        - We treat ``year_index`` as an offset from the first year on FaIR timebounds.
        - This avoids silent misalignment if the caller constructed year_index as (MY - FAIR_START_Y)
          but the FaIR instance was initialized with a different start year.

        For annual grids:
          timebounds[0] = Y0, timepoints[0] = Y0 + 0.5
          => pulse_year = Y0 + year_index
        """
    ...
    def get_gas_deltaC_t(self, specie: str, pulse_size: float = 1.0, store: bool = True):
        """
        Compute prospective concentration response to a one-year emissions pulse using a *separate*
        perturbed FaIR run.

        Definition
        ----------
        ΔC(t) = C_pert(t) - C_base(t), evaluated on FaIR ``timebounds``.

        Indexing convention (annual FaIR grids)
        --------------------------------------
        - Emissions live on ``timepoints`` (typically year + 0.5).
        - Concentrations live on ``timebounds`` (typically integer years).
        - A pulse in calendar year Y is applied at timepoint (Y + 0.5).
        - The *first realized* concentration response is at timebound index (Y + 1).
          We therefore define H=0 at (Y + 1) to avoid pre-pulse sampling at timebound Y.

        Returns
        -------
        dC_h : np.ndarray
            ΔC over horizon H=0..H_max (length nH = H_max*ts_per_year+1), shape (nH, ensemble/config).
        """
        ...
        if not hasattr(self.f, "timebounds") or not hasattr(self.f, "timepoints"):
            raise AttributeError(
                "FaIR object must provide both timebounds (concentration) and timepoints (emissions)."
            )
        ...
        # first realized concentration response on timebounds
        tb_resp0 = tb_pulse + 1

        # --- perturbed run on a fresh clone (critical for correctness) ---
        f_pert = copy.deepcopy(self.f)

        # baseline concentration from the SAME cloned object (guarantees alignment)
        conc_base = np.asarray(
            f_pert.concentration.loc[dict(scenario=self.scn, specie=specie)].values
        )

        # --- SANITY capture: concentration before pulse (same as conc_base, but explicit) ---
        debug_on = bool(getattr(self, "debug", True))
        if debug_on:
            conc0 = conc_base.copy()

        # apply one-year pulse on emissions (timepoints)
        em_slice = f_pert.emissions.loc[dict(scenario=self.scn, specie=specie)].copy()
        em_vals = np.asarray(em_slice.values)

        if debug_on:
            em_vals_before = em_vals.copy()

        if em_vals.ndim == 1:
            base_em_med = float(np.median(em_slice.values[tp_pulse]))
        ...
        f_pert.emissions.loc[dict(scenario=self.scn, specie=specie)] = em_slice
        ...
        # --- SANITY: does concentration change at all due to the emissions pulse? ---
        if debug_on:
            conc1 = conc_pert
            conc_diff_max = float(np.nanmax(np.abs(conc1 - conc0)))
        ...
        dC = conc_pert - conc_base  # on timebounds

        # --- horizon slice (H=0..H_max) ---
        nH = self.H_max * self.ts_per_year + 1
        tb1 = tb_resp0 + nH
        if tb1 > dC.shape[0]:
            raise ValueError(
        ...
        # debug prints for notebook inspection
        if debug_on:
    ...
    def get_gas_IRF_fair_t(
        ...
        """
        Prospective impulse response function (dimensionless), computed from FaIR reruns.
        IRF(t) = ΔC(t) / ΔC(t0)
        """
